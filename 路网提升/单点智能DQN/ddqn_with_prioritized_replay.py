...
class DQNPrioritizedReplay:
    ...
    # def get_policy(self, observation,last_action):
    #     observation = observation[np.newaxis, :]
    #     actions_value = self.sess.run(self.q_eval, feed_dict={self.s: observation})
    #     lower = max(0, last_action - 2)
    #     upper = min(12, last_action + 2)
    #     action = int(np.argmax(actions_value[0, lower:upper + 1]))
    #     return action
    def get_policy(self, observation,last_action):
        observation = observation[np.newaxis, :]
        actions_value = self.sess.run(self.q_eval, feed_dict={self.s: observation})
    ...
    def learn(self):
        ...
        save_path = saver.restore(self.sess, 'net/save_net.ckpt')
        
        if self.learn_step_counter % self.replace_target_iter == 0:
            '''
            saver = tf.train.Saver()
            
            save_path = saver.save(self.sess, 'net/save_net.ckpt')
            # 输出保存路径
        ...
        # if self.learn_step_counter == 103530:
        #     self.epsilon = 0.3
