...
class Division():
    ...
    def get_interval_num(self,row):
        flag = ""
        for i in range(len(self.p1)):
            if row['相位1'] in self.p1[i]:
                flag += str(i)
        for i in range(len(self.p2)):
            if row['相位2'] in self.p2[i]:
                flag += str(i)
        for i in range(len(self.p3)):
            if row['相位3'] in self.p3[i]:
                flag += str(i)
        return flag
    ...
    def run(self):
        ...
        for i in directoryFiles:
            ...
            self.p1=gen_phase(X,Qmax1,qmax1)
            self.p2=gen_phase(X,Qmax2,qmax2)
            self.p3=gen_phase(X,Qmax3,qmax3)

            for index,row in csv_data.iterrows():
                flag=self.get_interval_num(row)
                if index==0:
                    timeInterval = []
