...
#本实验支持针对c语言程序的语法，使用汤普森算法支持简单运算，较复杂的情况使用RE包内方法完善功能

########################################################################################
#汤普森算法所支持的C语法如下，支持对词法错误的错误抛出
#@表示数字
#支持以字母开头的字母下划线组合变量名
...
class Token(object):
    ...
    def __init__(self):
        ...
        self.lineno = 1
########################################################################################
#使用RE工具包的Scanner所支持的保留字，本RE算法暂时不支持报错
        #关键字
...
def NFA(s,label):#s:RE，label:相应的类别
    Statements=list()#NFA的节点，用字典表表示每个节点，key表示边，null表示ε边
    start=list()#起始节点
    end=list()#终节点
    stack = Stack()
    StateNums = 0#NFA总节点数
    ...
    for i in s:
        if i == '|':
            start.append(StateNums+1)#连接后，删除原来的两个起始节点，生成的新起始节点放入列表
            start.remove(stack.top()[0])
            start.remove(stack.top2()[0])
            a={"null":[stack.top()[0],stack.top2()[0]]}#将新起始节点连接两条ε边到原来两个起始节点
            b={"null":[]}

            end.append(StateNums+2)#删除终节点，生成新的终节点
            end.remove(stack.top()[1])
            end.remove(stack.top2()[1])
            Statements[stack.top()[1]-1]["null"].append(StateNums+2)#原来的终节点连接ε边到新的终节点
            Statements[stack.top2()[1]-1]["null"].append(StateNums+2)
            stack.pop()#弹出原来的关系
            stack.pop()

            Statements.append(a.copy())#新产生的两个节点
            Statements.append(b.copy())

            stack.push((StateNums+1,StateNums+2))#压进新的关系
            StateNums = StateNums+2
        elif i == '·':
            Statements[stack.top2()[1]-1]["null"].append(stack.top()[0])#连接两个·关系
            start.remove(stack.top()[0])
            end.remove(stack.top2()[1])
            ...
            Statements[stack.top()[1]-1]["null"].append(stack.top()[0])
            Statements[stack.top()[1]-1]["null"].append(StateNums+2)

            a = {"null":[StateNums+2,stack.top()[0]]}
            b = {"null":[]}

            start.append(StateNums+1)
            end.append(StateNums+2)

            Statements.append(a.copy())
            Statements.append(b.copy())

            stack.pop()
            stack.push((StateNums+1,StateNums+2))
            StateNums = StateNums+2

        else:
            a={i:StateNums+2,"null":[]}#生成两个新节点 a—i—>b
            b={"null":[]}
            start.append(StateNums+1)#把两个节点分别送入开始结束节点
            end.append(StateNums+2)

            Statements.append(a.copy())#把两个节点的拷贝放入节点集
            Statements.append(b.copy())
            stack.push((StateNums+1,StateNums+2))#将该元素开始结束节点关系压栈
            StateNums = StateNums+2
            elements.add(i)

    return Statements,start,end,StateNums,elements,label

#合并n个NFA
#(list:每个NFA的节点个数，list:每个NFA字典，starts:每个NFA的起始节点,ends:每个NFA的终节点,labels:每个NFA的标签)
def nNFA(Statements,NFAs,starts,ends,labels):
    for i in range(1,len(NFAs)):#将NFA节点数量合并， 如[3,3,3]变为[3,6,9]
        for j in range(0,len(NFAs[i])):
            for k in NFAs[i][j]:
                if k != "null":
                    for m in range(0,i):
                        NFAs[i][j][k] = NFAs[i][j][k] + Statements[m]
                else:
                    if NFAs[i][j][k]:
                        for m in range(0,i):
                            for n in range(0,len(NFAs[i][j][k])):
                                NFAs[i][j][k][n] = NFAs[i][j][k][n] + Statements[m]
        for x in range(i,len(starts)):#将开始节点数量合并
            for y in range(0,len(starts[x])):
                    starts[x][y] = starts[x][y] + Statements[i-1]
        for x in range(i,len(ends)):#将终节点数量合并
            for y in range(0,len(ends[x])):
                    ends[x][y] = ends[x][y] + Statements[i-1]
...
#状态间扩展
def betweenExpension(edge,core,NFA):
    num=0
    newcore = set()
    for i in list(core):
        ...
        if p != -1:
            newcore.add(p)
    newcore = innerExpension(newcore,NFA)
    return newcore
...
def DFA(NFA,start,end,elements,labels):
    ...
    queue.append(innerExpension(start,NFA))#队列的第一个元素为起始状态
    DFA_states_num.append(queue[0].copy())#起始状态加入DFA状态

    while queue:
        for i in elements:#遍历所有边
            x = betweenExpension(i,queue[0],NFA)
            if x:
    ...
    return Start,End,DFA_states_dict,all_labels
...
def make():
    ...
    Start,End,DFA_states_dict,all_labels=DFA(all_NFAs,all_starts,all_ends,list(set(e)),labels)

    rw1 = Start
    rw2 = End
    rw3 = DFA_states_dict
    rw4 = all_labels
    output_hal1 = open("Start.pkl", 'wb')
    output_hal2 = open("End.pkl", 'wb')
    output_hal3 = open("DFA", 'wb')
    output_hal4 = open("all.pkl", 'wb')
...
#(内容，指向当前字符的指针，行数，生成的文件名)
    ...
    all_labels = dict()
    with open("Start.pkl",'rb') as f1:
        Start = pickle.loads(f1.read())
    for i in Start:
        start_state = i
    with open("End.pkl",'rb') as f2:
        End = pickle.loads(f2.read())
